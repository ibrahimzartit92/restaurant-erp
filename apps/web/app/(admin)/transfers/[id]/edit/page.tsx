...
export default async function EditTransferPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [transferResult, branchesResult, warehousesResult, itemsResult, currencySettings] = await Promise.all([
  ...
  return (
    <>
      <PageHeader title="تعديل تحويل" description="حدث بيانات التحويل والمواد والكميات والتكلفة مع الحفاظ على نفس أسلوب الإدارة الحالي." />
      {transferResult.error || branchesResult.error || warehousesResult.error || itemsResult.error ? (
        <p className="notice">{transferResult.error ?? branchesResult.error ?? warehousesResult.error ?? itemsResult.error}</p>
      ) : null}
      <BranchTransferForm
